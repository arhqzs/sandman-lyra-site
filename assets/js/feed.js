/* ============================================================
   ...
   Renders Studio posts onto any page:
     <div class="feed" data-sl-feed="SECTION"></div>
   Optional filter bars:
     data-sl-filters     → filter by category (the Live page)
     data-sl-subfilters  → filter by sub-tab   (section pages)
   Reads window.SANDMAN_POSTS; sub-tab order from window.SANDMAN_SECTIONS.
   ============================================================ */
(function () {
  "use strict";
  var DATA = window.SANDMAN_POSTS || window.SANDMAN_LIVE || [];
  var SECTIONS = window.SANDMAN_SECTIONS || {};
  ...
  function postHTML(post) {
    ...
    return '' +
      '<article class="post" id="' + esc(post.id) + '">' +
        ...
        mediaHTML(post) +
        '<div class="post-body">' +
          (post.subsection ? '<span class="post-sub">' + esc(post.subsection) + "</span>" : "") +
  ...
  Array.prototype.forEach.call(feeds, function (feed) {
    ...
    var mode = feed.hasAttribute("data-sl-filters") ? "cat"
      : (feed.hasAttribute("data-sl-subfilters") ? "sub" : null);
    ...
    function presentValues() {
      ...
      var seen = {}; all.forEach(function (p) { if (p.subsection) seen[p.subsection] = 1; });
      var ordered = (SECTIONS[section] || []).filter(function (x) { return seen[x]; });
      Object.keys(seen).forEach(function (x) { if (ordered.indexOf(x) === -1) ordered.push(x); });
      return ordered;
    }

    var values = mode ? presentValues() : [];
    var bar = null;
    ...
    function renderBar() {
      if (!bar) return;
      var btns = ['<button class="filter-btn' + (active === "All" ? " active" : "") + '" data-val="All">All</button>'];
      presentValues().forEach(function (v) {
        btns.push('<button class="filter-btn' + (active === v ? " active" : "") + '" data-val="' + esc(v) + '">' + esc(v) + "</button>");
      });
      bar.innerHTML = btns.join("");
    }
    function list() {
      if (active === "All") return all;
      if (mode === "cat") return all.filter(function (p) { return (p.categories || []).indexOf(active) !== -1; });
      return all.filter(function (p) { return p.subsection === active; });
    }
    function render() {
      var l = list();
      feed.innerHTML = l.length ? l.map(postHTML).join("") : '<p class="feed-empty">Nothing posted here yet — check back soon.</p>';
    }
    feed._sl = { mode: mode, setActive: function (v) { active = v; renderBar(); render(); } };
    renderBar();
    render();
  });
  ...
  document.addEventListener("click", function (e) {
    ...
    if (fb) {
      ...
      if (theFeed && theFeed._sl) { theFeed._sl.setActive(fb.getAttribute("data-val")); window.scrollTo({ top: 0, behavior: "smooth" }); }
      return;
    }
    var tag = e.target.closest ? e.target.closest(".post-tags .tag") : null;
    if (tag) {
      var fEl = tag.closest("[data-sl-feed]");
      if (fEl && fEl._sl && fEl._sl.mode === "cat") { fEl._sl.setActive(tag.getAttribute("data-cat")); }
      return;
    }
